...
import {
  ChevronRight, Package, HelpCircle, Heart, Wallet,
  MapPin, User, Gift, CreditCard, Bell, Info, Star, RefreshCw, LogOut, Shield, Phone, Truck, Moon, Sun
} from 'lucide-react';
...
export default function Profile() {
  ...
  useEffect(() => { base44.auth.me().then(setProfileUser); }, []);

  const { data: addresses = [] } = useQuery({
    queryKey: ['profile-addresses', profileUser?.email],
    queryFn: () => base44.entities.Address.filter({ user_email: profileUser.email }),
  ...
  const menuItems = [
    ...
    { icon: MapPin, label: 'Saved Addresses', path: '/profile/addresses', badge: addresses.length > 0 ? `${addresses.length} Address${addresses.length > 1 ? 'es' : ''}` : null },
